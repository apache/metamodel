...
public class Neo4jDataContextTest extends Neo4jTestCase {
	...
	protected void setUp() throws Exception {
		...
		if (isConfigured()) {
			...
			requestWrapper = new Neo4jRequestWrapper(httpClient, httpHost,
					getUsername(), getPassword(), getServiceRoot());
		}
	}
	...
	public void testSelectQueryWithProjection() throws Exception {
		...
		{
			CompiledQuery query = strategy.query().from("JUnitLabel")
	...
	@Test
	public void testSelectQueryWithProjectionAndRelationships()
			throws Exception {
		if (!isConfigured()) {
			System.err.println(getInvalidConfigurationMessage());
			return;
		...
		{
			CompiledQuery query = strategy.query().from("JUnitPerson")
			...
			try (DataSet dataSet = strategy.executeQuery(query)) {
				...
				assertEquals("Row[values=[Helena, null]]", rows.get(0)
						.toString());
				assertEquals("Row[values=[Philomeena, null]]", rows.get(1)
						.toString());
				assertEquals("Row[values=[Tomasz, " + bookNodeId + "]]", rows
						.get(2).toString());
			}
		}
		{
			CompiledQuery query = strategy.query().from("JUnitPerson")
					.select("rel_HAS_READ#rating").compile();
			try (DataSet dataSet = strategy.executeQuery(query)) {
				List<Row> rows = new ArrayList<>();
				while (dataSet.next()) {
					rows.add(dataSet.getRow());
	...
	public void testSelectAllQueryWithRelationships() throws Exception {
		...
		CompiledQuery query = strategy.query().from("JUnitPerson").selectAll()
				.compile();
		try (DataSet dataSet = strategy.executeQuery(query)) {
			List<Row> rows = new ArrayList<>();
			while (dataSet.next()) {
			...
			Collections.sort(rows, new Comparator<Row>() {
				...
				@Override
				public int compare(Row arg0, Row arg1) {
					return arg0.getValue(1).toString()
							.compareTo(arg1.getValue(1).toString());
				}
			});
			assertEquals(3, rows.size());
			assertEquals("Row[values=[" + helenaNodeId
					+ ", Helena, 100, null, null, null]]", rows.get(0)
					.toString());
			assertEquals("Row[values=[" + philomeenaNodeId
					+ ", Philomeena, 18, null, null, " + bookNodeId + "]]",
					rows.get(1).toString());
			assertEquals("Row[values=[" + tomaszNodeId + ", Tomasz, 26, "
					+ bookNodeId + ", 5, null]]", rows.get(2).toString());
		}
	}
	...
	public void testWhereClause() throws Exception {
		...
		{
			CompiledQuery query = strategy.query().from("JUnitPerson")
					.select("rel_HAS_READ#rating").where("name").eq("Tomasz")
					.compile();
			try (final DataSet dataSet = strategy.executeQuery(query)) {
				assertTrue(dataSet.next());
		...
		{
			CompiledQuery query = strategy.query().from("JUnitPerson")
					.select("rel_HAS_READ#rating").where("name")
	...
	public void testJoinWithRelationships() throws Exception {
		...
		CompiledQuery query = strategy.query().from(table1).and(table2)
				.select(personNameColumn, bookIdColumn, bookTitleColumn)
				.where(personHasReadColumn).eq(bookIdColumn).compile();
		...
		try (final DataSet dataSet = strategy.executeQuery(query)) {
			...
			assertEquals("Row[values=[Tomasz, " + bookNodeId
					+ ", Introduction to algorithms]]", rows.get(0).toString());
		}
	}
	...
	public void testFirstRowAndLastRow() throws Exception {
		...
		try (final DataSet ds = dc.query().from("JUnitLabel").select("name")
				.and("age").firstRow(2).execute()) {
			assertTrue("Class: " + ds.getClass().getName(),
					ds instanceof Neo4jDataSet);
			assertTrue(ds.next());
	...
	public void testCountQuery() throws Exception {
		...
		// insert a few records
		{
			requestWrapper
					.executeCypherQuery("CREATE (n:JUnitLabel { name: 'John Doe', age: 30 })");
			requestWrapper
					.executeCypherQuery("CREATE (n:JUnitLabel { name: 'Sofia Unknown', gender: 'F' })");
		}
		...
		try (final DataSet ds = dc.query().from("JUnitLabel").selectCount()
				.where("name").eq("John Doe").execute()) {
			assertTrue(ds.next());
			final Row row = ds.getRow();
			assertFalse(ds.next());
