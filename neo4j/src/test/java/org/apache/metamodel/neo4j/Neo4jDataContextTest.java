...
public class Neo4jDataContextTest extends Neo4jTestCase {
	...
	public void testSelectQueryWithProjectionAndRelationships() throws Exception {
		...
		try (DataSet dataSet1 = strategy.executeQuery(query1)) {
			...
			assertEquals("Row[values=[Helena, null]]", dataSet1Rows.get(0)
					.toString());
			assertEquals("Row[values=[Philomeena, null]]", dataSet1Rows.get(1)
					.toString());
	...
	public void testCountQuery() throws Exception {
		...
		try (final DataSet ds1 = dc.query().from("JUnitLabel").selectCount()
		...
		try (final DataSet ds2 = dc.query().from("JUnitLabel").selectCount()
				.execute()) {
			assertTrue(ds2.next());
			final Row row2 = ds2.getRow();
			assertFalse(ds2.next());
